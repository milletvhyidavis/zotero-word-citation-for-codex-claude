...
`users/0` means "the local user"; the real library identity comes from each item's `library` object (`{"type": "user", "id": 9676164}`) — the URI is built from that object only. If `id` is not a positive number (unsynced local library), `item_uri()` refuses; use a URI from an existing Zotero-generated field or sync the library.
