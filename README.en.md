...
Every script works on its own and has `--help`. `S` below is the skill's `scripts/` folder:

```bash
S=skills/zotero-word-live-citations/scripts
```

Search literature and match it against your Zotero library:
...
If some papers are not in your library: build an import file, **select the target collection in Zotero**, then import and match again:
...
In `draft.md`, markers such as `[@ref:C1; @ref:C3]` mark where citations go. Convert to Word, insert citations and check the result:
...
Finally open `draft-zotero-cited.docx` in Word and click **Zotero → Refresh**.

On PowerShell, set the variable with `$S = "skills\zotero-word-live-citations\scripts"` and write `python "$S\search_literature.py" …`.

Optional environment variables: `ZWLC_MAILTO` (contact email sent with searches), `NCBI_API_KEY` (higher PubMed rate limit), `ZOTERO_LOCAL_BASE_URL` (Zotero on a non-default port).

Full reference: [usage guide](docs/en/usage.md).
